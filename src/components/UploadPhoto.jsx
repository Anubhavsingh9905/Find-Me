...
const UploadPhoto = () => {
  ...
  useEffect(() => {
    ...
    const fetchPhoto = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/photo/${user._id}`, { withCredentials: true });
  ...
  let handleUpload = async() => {
    ...
    try{
      await axios.post(`http://localhost:4000/photo/upload/${user._id}`, fromdata, { headers: { "Content-Type": "multipart/form-data" } })
  ...
  let handleDelete = async() => {
    ...
    try{
      await axios.post(`http://localhost:4000/photo/delete/${user._id}`)
      await refreshAuth();
      setPhotoUrl(null);
      setLoading(false);
      console.log(res);
